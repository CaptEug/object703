class_name BuildingTurretEditingSystem
extends RefCounted
...
func start_block_placement(scene_path: String):
	if not is_turret_editing_mode or not selected_building:
		return
...
func try_place_turret_block():
	...
	if selected_building:
		selected_building.update_building()
...
func validate_snap_config_before_placement() -> bool:
	...
	elif turret_snap_config.has("building_point") and turret_snap_config.has("ghost_point"):
		var building_point: Connector = turret_snap_config.building_point
		var ghost_point: Connector = turret_snap_config.ghost_point
		
		if not is_instance_valid(building_point) or not is_instance_valid(ghost_point):
			return false
		
		if not can_points_connect_with_rotation_for_turret(building_point, ghost_point, 0):
			return false
...
func get_turret_at_position(position: Vector2) -> TurretRing:
	...
	for collision in result:
		var block = collision.collider
		if block is TurretRing and block.get_parent() == selected_building:
			return block
	return null

# === 炮塔外吸附网格计算函数（与建筑系统完全一致）===
func calculate_rotated_grid_positions_turret(building_point: Connector, ghost_point: Connector):
	# 这个函数与建筑编辑系统的 calculate_rotated_grid_positions 完全一致
	var grid_positions = []
	var grid_block = {}
	...
	var location_v = building_point.location
	
	var rotation_b = building_point.find_parent_block().base_rotation_degree
	var grid_b = {}
	var grid_b_pos = {}
	
	for key in current_editing_turret.turret_grid:
		if current_editing_turret.turret_grid[key] == building_point.find_parent_block():
	...
	grid_connect_g = get_connection_offset_turret(connect_pos_v, building_point.rotation, building_point.find_parent_block().base_rotation_degree)
...
func calculate_aligned_rotation_for_turret_block(building_block: Block) -> float:
	var world_rotation = building_block.global_rotation
	var self_rotation = deg_to_rad(building_block.base_rotation_degree)
...
func can_points_connect_with_rotation_for_turret(point_a: Connector, point_b: Connector, ghost_rotation: float) -> bool:
	...
	var building_point_direction = wrapf(point_a.global_rotation, -PI, PI)
	
	# 计算角度差
	var angle_diff = abs(wrapf(building_point_direction - ghost_point_direction, -PI, PI))
