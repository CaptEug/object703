...
func _refresh_item_storage(storage: ItemStorage) -> void:
	var item_ids: Array = storage.items.keys()
	item_ids.sort()
...
func _refresh_allowed_items() -> void:
	var item_ids := _get_allowed_items()
	if item_ids.is_empty():
...
func _make_item_row(item_id: String, amount_text: String) -> HBoxContainer:
...
func _get_addable_items() -> Array[String]:
	...
	if target_block is ItemStorage:
		compatible = (target_block as ItemStorage).get_compatible_item_ids()
	elif target_block is LiquidStorage:
		compatible = (target_block as LiquidStorage).get_compatible_item_ids()
	for item_id: String in _get_allowed_items():
		compatible.erase(item_id)
	return compatible

func _remove_allowed_item(item_id: String) -> void:
	if target_block is ItemStorage:
		(target_block as ItemStorage).remove_allowed_item(item_id)
	elif target_block is LiquidStorage:
		(target_block as LiquidStorage).remove_allowed_item(item_id)
...
func _build_add_tree() -> void:
	...
	var categories := {}
	for item_id: String in _get_addable_items():
		var category_name := _get_item_category(item_id)
...
func _get_item_category(item_id: String) -> String:
	if ItemDB.has_subclass(item_id, ItemDB.ItemSubclass.RAW_ORE):
		return "Raw Ores"
	if ItemDB.has_subclass(item_id, ItemDB.ItemSubclass.AMMO):
		return "Ammunition"
	if ItemDB.has_subclass(item_id, ItemDB.ItemSubclass.FUEL):
		return "Fuels"
	if target_block is LiquidStorage:
