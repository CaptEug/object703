...
func get_bit_mask(cell: Vector2i, shaft_grid: Dictionary) -> int:
	var mask := 0
	if shaft_grid.has(cell + Vector2i.UP):
		mask |= 1
	if shaft_grid.has(cell + Vector2i.RIGHT):
		mask |= 2
	if shaft_grid.has(cell + Vector2i.DOWN):
		mask |= 4
	if shaft_grid.has(cell + Vector2i.LEFT):
		mask |= 8
	return mask


func update_sprite() -> void:
	match get_bit_mask(origin_cell, vehicle.power_system.shaft_grid):
		5:
